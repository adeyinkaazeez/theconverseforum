...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Sport_Articles.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Sport_Articles.Status.DRAFT)
    

class Sport_Articles(models.Model): 
    User = get_user_model()
    ...
    likes = models.ManyToManyField(User, related_name='liked_sport_article_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Sport Article', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('sport_articles:post_detail',
    ...
    def get_comments(self):
        return self.sports_comments.filter(sports_parent=None).filter(active=True)
    
    
class Sport_Article_Comments(models.Model):
    User = get_user_model()    
    post = models.ForeignKey(Sport_Articles,                            
                             on_delete=models.CASCADE,                             
                             related_name='sports_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='sportsuser_comments' )     
    ...
    sports_parent = models.ForeignKey('self', on_delete=models.CASCADE, 
                                       related_name="sports_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_sport_article_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_sport_article_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Sport Article Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Sport_Article_Comments.objects.filter(sports_parent=self).filter(active=True)
