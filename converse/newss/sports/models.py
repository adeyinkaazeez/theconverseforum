...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Sports.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Sports.Status.DRAFT)
...
class Sports(models.Model): 
    ...
    likes = models.ManyToManyField(User, related_name='liked_sport_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation') 
    section_name = models.CharField(max_length=50, default='Sport', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('sports:post_detail',
    ...
    def get_comments(self):
        return self.sport_comments.filter(sport_parent=None).filter(active=True)
    
    
class Sport_Comments(models.Model):
    User = get_user_model()    
    post = models.ForeignKey(Sports,                            
                             on_delete=models.CASCADE,                             
                             related_name='sport_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='sportuser_comments' )    
    ...
    sport_parent = models.ForeignKey('self', on_delete=models.CASCADE, 
                                       related_name="sport_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_sport_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_sport_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Sport Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Sport_Comments.objects.filter(sport_parent=self).filter(active=True)
