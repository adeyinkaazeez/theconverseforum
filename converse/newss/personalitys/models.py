...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Personalities.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Personalities.Status.DRAFT)
...
class Personalities(models.Model): 
    ...
    author = models.ForeignKey(User,                             
                                ...
                                related_name='personality_posts' )
    body = RichTextField()
    ...
    likes = models.ManyToManyField(User, related_name='liked_personality_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation') 
    section_name = models.CharField(max_length=50, default='Personalities', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('personalitys:post_detail',
    ...
    def get_comments(self):
        return self.personality_comments.filter(personal_parent=None).filter(active=True)
    
class Personality_Comments(models.Model): 
    User = get_user_model()      
    post = models.ForeignKey(Personalities,                            
                             on_delete=models.CASCADE,                             
                             related_name='personality_comments') 
    
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='personaluser_comments' )   
    ...
    personal_parent = models.ForeignKey('self', on_delete=models.CASCADE, related_name="personal_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_personality_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_personality_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Personalities Comment', editable=False)
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Personality_Comments.objects.filter(personal_parent=self).filter(active=True)
