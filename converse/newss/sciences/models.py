...
class PublishedManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Science_AND_Techs.Status.PUBLISHED) #status field must be set to PUBLISHED too
    
class DraftManager(models.Manager):    
    def get_queryset(self):        
        return super().get_queryset().filter(status=Science_AND_Techs.Status.DRAFT)
    

class Science_AND_Techs(models.Model): 
    User = get_user_model()
    ...
    likes = models.ManyToManyField(User, related_name='liked_science_post', blank=True)
    total_likes = models.PositiveIntegerField(default=0)
    hit_count_generic = GenericRelation(HitCount, object_id_field='object_pk', 
                                        related_query_name='hit_count_generic_relation')
    section_name = models.CharField(max_length=50, default='Science & Tech', editable=False) 
    ...
    def get_absolute_url(self):
        return reverse('sciences:post_detail',
    ...
    def get_comments(self):
        return self.st_comments.filter(science_parent=None).filter(active=True)
    
    
class Science_and_Tech_Comments(models.Model): 
    User = get_user_model()   
    post = models.ForeignKey(Science_AND_Techs,                            
                             on_delete=models.CASCADE,                             
                             related_name='st_comments') 
    
    name = models.ForeignKey(User,  on_delete=models.CASCADE,            
                                related_name='scienceuser_comments' )     
    ...
    science_parent = models.ForeignKey('self', on_delete=models.CASCADE, 
                                       related_name="science_replies", null=True)
    edited = models.BooleanField(default=False)
    edited_date = models.DateTimeField(auto_now=True)
    likes = models.ManyToManyField(User, related_name='liked_science_comment', blank=True)
    dislikes = models.ManyToManyField(User, related_name='disliked_science_comment', blank=True)
    section_name = models.CharField(max_length=50, default='Science & Tech Comment', editable=False) 
    class Meta:        
        ordering = ['created']        
        indexes = [            
    ...
    def get_comments(self):
        return Science_and_Tech_Comments.objects.filter(science_parent=self).filter(active=True)
