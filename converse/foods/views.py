...
from .models import Foods, Foods_Comment 
...
def Create_Post(request): 
    ...
    if request.method == 'POST':
       ...
       if form.is_valid():
          ...
          return redirect('foods:create_post')
    else:
       form = PostForm(request.POST or None)
    return render(request, 'food_post/create_post.html',
                  {'form':form})


def post_list(request, tag_slug=None):
     food_posts = all_Food_Posts(request)
     ...
     return render(request, 'food_post/food_home.html', context )

def list_load_Food_posts_view(request):
   food_posts = all_Food_Posts(request)
   context = {'food_posts':food_posts}
   return render(request, "food_post/partials/food_post_list.html", context)

def all_Food_Posts(request):
   objs = Foods.published.select_related('author').all()
...
def like_post(request, pk):
   post = Foods.objects.get(pk=pk)
   if request.method =='POST':
      if not post.likes.filter(id=request.user.pk).exists():
         post.likes.add( request.user)
         post.save()
         create_action(request.user, 'endorsed', post)
         return render(request, 'food_post/partials/like.html', context={'post':post})
      else:
...
def Update_Post(request, pk=None):
   ...
   if request.method == 'POST':
      ...
      if form.is_valid():
         ...
         return render(request,   'food_post/addInForum.html', {'post':post})
   else:
      form = PostForm(instance=post)
   return render(request,'food_post/addInForum.html', {'form':form, 'post':post})


#deleting post
def delete_post(request, pk=None):
   post = Foods.objects.get(pk=pk).delete()
   create_action(request.user, 'removed', post)
   return render(request, 'food_post/delete.html')


def post_detail(request, year, month, day,post):    
    post = get_object_or_404(Foods,                                                          
                             status=Foods.Status.PUBLISHED,
                              slug=post,
                              publish__year=year,                             
                              publish__month=month,                            
                            publish__day=day) 
     # List of active comments for this post    
    comments = post.food_comments.filter(active=True) 
    ...
    similar_posts = Foods.published.filter(tags__in=post_tags_ids)\
                                            .exclude(id=post.id)
    ...
    most_viewed = Foods.objects.filter(status=Foods.Status.PUBLISHED).order_by( 'hit_count_generic')[:5]  
    ...
    return render(request, 'food_post/food_details.html', context)


def post_share(request, post_id):    
   # Retrieve post by id    
   post = get_object_or_404(Foods, id=post_id, status=Foods.Status.PUBLISHED)
   ...
   if request.method == 'POST':        
    ...
    form = EmailPostForm()    
   return render(request,'food_comment/share.html', 
                 {'post': post,
                  'form': form,  
                  'sent': sent})
...
def post_comment(request, post_id):    
  ...
  if request.method == 'POST':
     ...
     form = CommentForm()  
  return render(request , 'food_comment/comment_confirm.html',                            
                {'post': post,                            
                 'form': form,                            
                 'comment': comment})

def Update_Comment(request, pk=None):
   comment = Foods_Comment.objects.get(pk=pk)
   ...
   return render(request, 'food_comment/comment_edit_form.html',
                   { 'form':form,
                   'comment':comment,})

#deleting comment
def delete_comment(request, pk=None):
   comment = Foods_Comment.objects.get(pk=pk).delete()
   create_action(request.user, 'removed ', comment)
   return render(request, 'food_comment/delete.html')
...
def reply_page(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            reply.post = Foods(id=post_id)
            reply.food_parent = Foods_Comment(id=parent_id)
            comment = Foods_Comment.objects.get(id=parent_id)
...
class Like_comment(LoginRequiredMixin, View):
   def post(self, request, post_pk=None, pk=None, *args, **kwargs):
      post = Foods.objects.get(pk=post_pk)
      comment = Foods_Comment.objects.get(pk=pk)
      author = comment.name
      if request.method=="POST":
          is_dislike = False
          ...
          if not is_like:
             ...
             return render(request,'food_comment/partials/like.html', context={'comment':comment, 'post':post})
          if is_like:
