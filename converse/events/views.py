...
from .models import Events_Comment , Events
...
def Create_Post(request):
    ...
    if request.method == 'POST':
       ...
       if form.is_valid():
          ...
          return redirect('events:create_post')
    else:
       form = PostForm(request.POST or None)
    return render(request, 'event_post/create_post.html',
                  {'form':form})

def post_list(request, tag_slug=None):
     event_posts = all_Event_Posts(request) 
     ...
     return render(request, 'event_post/event_home.html', context)

def list_load_Event_posts_view(request):
   event_posts = all_Event_Posts(request)
   context = {'event_posts':event_posts}
   return render(request, "event_post/partials/event_post_list.html", context)

def all_Event_Posts(request):
   objs = Events.published.select_related('author').all()
...
def like_post(request, pk):
   post = Events.objects.get(pk=pk)
   if request.method =='POST':
      if not post.likes.filter(id=request.user.pk).exists():
         post.likes.add( request.user)
         post.save()
         create_action(request.user, 'endorsed', post)
         return render(request, 'event_post/partials/like.html', context={'post':post})
      else:
...
def Update_Post(request, pk=None):
   ...
   if request.method == 'POST':
      ...
      if form.is_valid():
         ...
         return render(request,   'event_post/addInForum.html', {'post':post})
   else:
      form = PostForm(instance=post)
   return render(request, 'event_post/addInForum.html', {'form':form, 'post':post})



#deleting post
def delete_post(request, pk=None):
   post = Events.objects.get(pk=pk).delete()
   return render(request, 'event_post/delete.html')


def post_detail(request, year, month, day,post):    
    post = get_object_or_404(Events,                                                          
    ...
    comments = post.event_comments.filter(active=True) 
    ...
    similar_posts = Events.published.filter(tags__in=post_tags_ids)\
                                            .exclude(id=post.id)
    ...
    most_viewed = Events.objects.filter(status=Events.Status.PUBLISHED).order_by( 'hit_count_generic')[:5]
    ...
    return render(request, 'event_post/event_detail.html', context)


def post_share(request, post_id):    
   # Retrieve post by id    
   post = get_object_or_404(Events, id=post_id, status=Events.Status.PUBLISHED)
   ...
   if request.method == 'POST':        
    ...
    form = EmailPostForm()    
   return render(request, 'event_comment/share.html', 
                 {'post': post,
                  'form': form,  
                  'sent': sent})
...
def post_comment(request, post_id):    
  ...
  if request.method == 'POST':
     ...
     form = CommentForm() 
  return render(request , 'event_comment/comment_confirm.html',                           
                {'post': post,                            
                 'form': form,                            
                 'comment': comment})
...
def Update_Comment(request, pk=None):
   comment = Events_Comment.objects.get(pk=pk)
   ...
   return render(request, 'event_comment/comment_edit_form.html',
                   { 'form':form,
                   'comment':comment,
 })
...
def delete_comment(request, pk=None):
   comment = Events_Comment.objects.get(pk=pk).delete()

   return render(request, 'event_comment/delete.html')
...
def reply_page(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            comment = Events_Comment.objects.get(id=parent_id)
            reply = form.save(commit=False)
                
    
            reply.post = Events(id=post_id)
            reply.event_parent = Events_Comment(id=parent_id)
...
class Like_comment(LoginRequiredMixin, View):
   def post(self, request, post_pk=None, pk=None, *args, **kwargs):
      post = Events.objects.get(pk=post_pk)
      comment = Events_Comment.objects.get(pk=pk)
      author = comment.name
      if request.method=="POST":
          is_dislike = False
          ...
          if not is_like:
             ...
             return render(request,'event_comment/partials/like.html', context={'comment':comment, 'post':post})
          if is_like:
